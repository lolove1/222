...
@Desc    :
            活动地址：
               https://personal-act.wps.cn/rubik2/portal/HD2025031821201822/YM2025040908558269?position=ad_rwzx_invite_test&token=d8bf0d576596cf020db8bde9b762644d
            核心功能：
                 1. WPS任务中心：每日签到、自动完成所有任务、自动抽奖
                 2. 办公助手（办公助手 2026.03.16结束）：每日签到、自动完成所有任务、自动抽奖
                 3. 天天领福利：每日签到、自动完成所有任务、自动抽奖
                 4. WPS超级会员小程序：签到
                 5. WPS挑战计划：自动完成所有任务、自动抽奖
            使用：
...
class RNL:
    ...
    def main(self):
        ...
        if self.global_config.get('task_center'):
            ...
            # 重试
            page_data = self.page_info()
            self.lottery_process(task_list= page_data["task_list"] or list())

            Utils.r_sleep(1.5)
            page_data = self.page_info()
            lottery_times = page_data["lottery_times"] or 0
            # 抽奖
            if lottery_times > 0:
        ...
        if self.global_config.get('lottery3'):
            ...
            lottery_times3 = page_info3['lottery_times'] or 0
            if lottery_times3 > 0:
        ...
        if self.global_config.get('challenge'):
            ...
            challenge_task_list=challenge_data['task_list'] or list()
